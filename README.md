...
最新[嘉豪角色喜剧实验](docs/character-comedy-iteration.md)仍未通过体验验收；局部笑点改善不等于整版好玩。

`codex/solo-scene-upgrade` 增加十五个校园、工作和朋友间的日常现场、三回合自由接招、名场面卡和同题分享，保留原有玩法。
**当前尚未通过内容一致性验收，不应生产发布。** 已完成真实模型与桌面／手机流程验证，但留出试玩仍发现角色身份和行动事实错误。

本轮去AI味调整见 [日常场景记录](docs/daily-scenes-iteration.md)。

启动和回归见 [TESTING.md](TESTING.md)，改进证据、失败样本、五分钟路线与回滚见 [本轮交付记录](docs/solo-scene-iteration.md)。普通小剧场不在数据库保存正文；用户确认分享后只保存所选片段与收场，七天有效，可撤销站内展示。下载或他人保存的副本无法远程撤回。
...
首页主循环是免注册的熟人异步联赛：创建房间、每日同题作答、MiniMax 即时判定、好友投票与 7 日积分结算。题库从 `/admin/prompts` 排期，每日轮次按上海时间锁榜。

- 未加入的访客只能读取房间与今日题目预览，不能读取成员答案。
- 自己完成作答后才解锁好友答案与投票；判定失败保留待重判状态，不伪造分数。
- 赛季结束 7 天后删除原句与投票身份映射；房间连续 30 天无活动才归档。
- 生产环境应设置独立的 `LEAGUE_RECOVERY_SECRET`，不要将真实密钥提交到仓库。

### 后台 AI 配置中心

观测台的“AI 配置”支持分别管理文字、视觉、图片和视频模型。API 密钥只写入、不回显，并使用 `AI_CONFIG_ENCRYPTION_KEY` 加密保存。新配置必须先通过服务端真实鉴权与连通测试，测试成功后才会生成一次性切换凭据；测试失败、凭据过期或重复使用都不会替换当前线上配置。
