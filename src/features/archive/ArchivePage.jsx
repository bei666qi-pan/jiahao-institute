...
const JIAHAO_TYPES = [
  ['自在极意豪', '豪气自行运转，越不解释越有戏。'],
  ['美式嘉豪', '走两步就像在拍自己的音乐短片。'],
  ['深情破碎豪', '嘴上说无所谓，歌单已经循环八遍。'],
  ['计算机嘉豪', '终端常驻，张口就是底层原理。'],
  ['股票嘉豪', '涨了早就说了，跌了长期价值。'],
  ['不懂装懂豪', '术语密度拉满，细问就是底层逻辑。'],
  ['小众优越豪', '不是冷门，只是你们暂时还不懂。'],
  ['潜伏嘉豪', '表面风平浪静，细节里全是伏笔。'],
  ['反向嘉豪', '越是否认自己嘉豪，豪气越难隐藏。'],
];

const NAILOONG_TYPES = [
  ['淡人型奶龙豪', '表面说都行，心里已经把一切看淡。', 'arms'],
  ['嘴硬型奶龙豪', '表面说没事，心里准备了三套解释。', 'toes'],
  ['先点外卖型奶龙豪', '世界可以迟到，但外卖最好准时。', 'umbrella'],
  ['抽象反应型奶龙豪', '正常答案路过了，你从旁边绕行。', 'cat'],
  ['镜头待机型奶龙豪', '嘴上说别拍，站位从来没有输过。', 'arms'],
...
export function ArchivePage({ history, onSelectHistory, onClear }) {
  ...
  return <main className="archive-page">
    ...
    <section className="persona-gallery">{items.map(([name, summary, asset], index) => <article key={name}><span>{String(index + 1).padStart(2, '0')}</span>{tab === 'nailoong' ? <img src={`/assets/nailoong/${asset}.webp`} alt=""/> : <div className="type-glyph">豪</div>}<h2>{name}</h2><p>{summary}</p></article>)}</section>
    <section className="history-section"><header><div><h2>我的鉴定记录</h2><p>记录只保存在当前浏览器。</p></div>{history.length ? <button type="button" className="text-button" onClick={onClear}>清空记录</button> : null}</header>{history.length ? <div className="history-grid">{history.map((item) => <button type="button" key={`${item.id}-${item.createdAt || ''}`} onClick={() => onSelectHistory(item)}><time>{item.createdAt ? new Date(item.createdAt).toLocaleDateString('zh-CN') : '旧记录'}</time><span><b>{item.jiahao?.score ?? item.score}</b><small>嘉豪</small></span><span><b>{item.nailoong?.score ?? '--'}</b><small>奶龙</small></span><strong>{item.nailoong?.archetype || item.type}</strong><Icon name="arrow"/></button>)}</div> : <div className="empty-archive"><Icon name="archive" size={38}/><strong>还没有鉴定记录</strong><p>第一份双指数档案正在等你。</p></div>}</section>
  </main>;
}
