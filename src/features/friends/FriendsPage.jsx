...
function LeagueRoom({ data, roomCode, nickname, onNickname, busy, error, notice, recoveryCode, onJoin, onShare, onReload, onBack }) {
  ...
  const submissionScope = `${roomCode}:${data.member?.memberId || 'preview'}:${season?.number || 0}:${round?.date || 'none'}`;
  const draftKey = `jiahao-league-draft:v1:${submissionScope}`;
  const [storedDraft] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem(draftKey) || 'null'); } catch { return null; }
  });
  const [answer, setAnswer] = useState(() => typeof storedDraft?.answer === 'string' ? storedDraft.answer.slice(0, 120) : '');
  const [consent, setConsent] = useState(false);
  const [shareAnswer, setShareAnswer] = useState(storedDraft?.shareAnswer === true);
  const [confirming, setConfirming] = useState(false);
  const [selectedAngle, setSelectedAngle] = useState('');
  const [localError, setLocalError] = useState('');
  const [working, setWorking] = useState(false);
  const [draftStatus, setDraftStatus] = useState('');
  const idempotencyKey = useMemo(() => /^[0-9a-f-]{36}$/i.test(storedDraft?.idempotencyKey || '') ? storedDraft.idempotencyKey : makeIdempotencyKey(), [submissionScope]);
  const currentDay = Math.min(7, Math.max(1, Number(season?.day) || 1));

  useEffect(() => {
    if (!data.isMember || !round) return;
    try {
      if (round.hasSubmitted || !answer) {
        sessionStorage.removeItem(draftKey);
        setDraftStatus('');
      } else {
        sessionStorage.setItem(draftKey, JSON.stringify({ answer, shareAnswer, idempotencyKey }));
        setDraftStatus('草稿已保存在当前标签页，刷新可恢复；交卷状态以服务器为准。');
      }
    } catch { setDraftStatus('此浏览器无法保存草稿，请暂勿刷新。'); }
  }, [draftKey, answer, shareAnswer, idempotencyKey, data.isMember, round?.hasSubmitted]);

  const syncAfterFailure = async (nextError) => {
    const refreshed = await onReload();
    if (refreshed?.round?.hasSubmitted) setLocalError('已收到服务器确认，今天已交卷。');
    else if (refreshed) setLocalError(nextError.message);
    else setLocalError('暂时无法确认服务器是否收到答案，请恢复网络后刷新核对；原句仍在当前页面。');
    setConfirming(false);
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const refreshed = await onReload();
      if (refreshed?.round?.hasSubmitted) trackProductEvent('lab_game_completed', { game: 'league_daily', character: round?.character, roundDay: season?.day });
    } catch (nextError) {
      await syncAfterFailure(nextError);
  ...
  const retryJudgement = async (submissionId = null) => {
    ...
    catch (nextError) { await syncAfterFailure(nextError); }
    finally { setWorking(false); }
  };
...
export function FriendsPage({ roomCode, latestResult, onNavigate, onRoomOpen, entryIntent }) {
  ...
  const [entryMode, setEntryMode] = useState(entryIntent === 'join' ? 'join' : 'create');
  ...
  const loadRoom = async (code) => { setError(''); try { const payload = await apiRequest(`/api/social/rooms/${encodeURIComponent(code)}`, { signal: AbortSignal.timeout(8_000) }); setRoomData(payload); trackProductEvent('challenge_opened', { roomType: payload.room?.roomType || 'friends' }); return payload; } catch (nextError) { setError(nextError.message); return null; } };
  useEffect(() => { if (roomCode) void loadRoom(roomCode); else void loadSession(); }, [roomCode]);
  useEffect(() => { if (entryIntent === 'join' && !roomCode) setEntryMode('join'); }, [entryIntent, roomCode]);
  ...
  if (roomCode && roomData?.room?.roomType === 'league') return <LeagueRoom key={`${roomCode}:${roomData.member?.memberId || 'preview'}:${roomData.season?.number || 0}:${roomData.round?.date || 'none'}`} data={roomData} roomCode={roomCode} nickname={nickname} onNickname={setNickname} busy={busy} error={error} notice={notice} recoveryCode={recoveryCode} onJoin={joinRoom} onShare={shareRoom} onReload={() => loadRoom(roomCode)} onBack={() => onNavigate('friends')}/>;
