  ...
  return <>
    ...
    <dialog ref={dialogRef} className="feedback-dialog" aria-labelledby="feedback-title" onClick={(event) => { if (event.target === dialogRef.current) dialogRef.current.close(); }}>
      <form onSubmit={submit}>
        ...
        <span className="feedback-kicker">有话直说</span><h2 id="feedback-title">哪儿不好玩？</h2><p>哪里卡住了，哪句不好笑，想玩什么，都能说。</p>
...
export function HaoPage({ onNavigate, onRoomOpen }) {
  ...
  useEffect(()=>{apiRequest('/api/scenes').then(v=>{setScenesEnabled(v.enabled===true);setScenes(v.scenes||[])}).catch(()=>{});},[]);
  const focusQuote = () => {
  ...
  return <main className="hao-page">
    <FeedbackEntry/>
    <header className="hao-hero league-first-hero"><div className="hao-hero-copy"><h1 aria-label={scenesEnabled ? "嘉豪之神挑战" : "每天一句，七天决出嘉豪之神"}>{scenesEnabled ? <span>嘉豪之神挑战</span> : <><span>每天一句</span><span>七天决出嘉豪之神</span></>}</h1><p>{scenesEnabled ? "嘉豪又把话说满了。你能让他改口吗？" : "发到微信群，好友 30 秒同题作答。AI 先判，群友再投，每晚锁榜。"}</p><div className="hero-actions"><button type="button" className="primary-button" onClick={() => scenesEnabled ? onNavigate('scenes','photo') : activeLeague ? onRoomOpen(activeLeague.code) : onNavigate('friends')}>{scenesEnabled ? '先接一招' : activeLeague ? '回到今天的好友联赛' : '创建 7 日好友联赛'} <Icon name="arrow"/></button><button type="button" className="outline-button" onClick={() => onNavigate('friends','join')}>输入房间码</button></div><small className="league-privacy-line"><Icon name="lock" size={15}/> {scenesEnabled ? "不用注册 · 聊三轮 · 分享前由你确认" : "免注册 · 答案只对房间成员可见 · 赛季后 7 天删除原句"}</small></div><figure className="hao-hero-media"><img src="/assets/jiahao/hao-universe-hero.webp" alt="巷子里笑得很开心的嘉豪" width="960" height="1200" fetchPriority="high"/><figcaption>嘴这么硬，你来试试。</figcaption></figure></header>
    {scenesEnabled ? <ScenePicker scenes={scenes} onChoose={id=>onNavigate('scenes',id)}/> : null}
    <section className="free-play-heading"><span>FREE PLAY</span><h2>自由玩</h2><p>鉴定一下，改句语录，或做张图。</p><button type="button" className="outline-button" onClick={() => onNavigate('assay')}>去做嘉豪鉴定 <Icon name="arrow"/></button></section>
    <HaoQuoteStudio inputRef={quoteInputRef}/>
    <section className="hao-play-rail"><article><div><h2>和好友比一局</h2><p>双方各出一句话，看看谁更豪。</p></div><button type="button" className="outline-button" onClick={() => setPkOpen((value) => !value)} aria-expanded={pkOpen}>{pkOpen ? '收起 PK' : '双人豪气 PK'} <Icon name="arrow"/></button></article><article className="hao-archive-door"><JiahaoPortrait variant={2} label="经典嘉豪人物图鉴预览" className="hao-archive-preview"/><div><h2>嘉豪出没图鉴</h2><p>六种嘉豪，看看你碰到过哪一种。</p></div><button type="button" className="outline-button" onClick={() => onNavigate('archive','preview')}>去看图鉴 <Icon name="arrow"/></button></article></section>
